...
impl DirNode {
    ...
    pub fn total_in_dirs_less_than_100k(&self) -> u32 {
        ...
        let to_add = if total < 100000 { total }
                     else { 0 };
        return to_add
            + &self.children
                   .iter()
...
pub fn run(input: &str) {
    ...
    println!("Total storage in directories less than 100k: {}",
             root.borrow().total_in_dirs_less_than_100k());
    println!("Size of smallest dir larger than threshold: {}",
...
fn size_of_dir_to_delete(root: &Rc<RefCell<DirNode>>) -> u32 {
    ...
    return root.borrow().size_of_smallest_dir_greater_than(space_to_free);
}

fn build_tree(input: &str, root: &Rc<RefCell<DirNode>>) {
    let mut current = Rc::clone(&root);
    for line in input.lines().skip(1) {  // skip first line "cd /"
        if line.contains("dir ") {
        ...
        } else if line.contains("$ cd ..") {  // move up to parent
            ...
            current = child;
        } else {  // add file size to current
            let filesize = line.split(' ').nth(0).unwrap().parse::<u32>().unwrap();
